...
| Self-tested quality | 9 rubrics + 51 golden cases with a deterministic scorer in CI, and a mechanical dev→main release gate that fails a promotion with no eval baseline. | [`evals/README.md`](evals/README.md) · [`scripts/release_gate.py`](scripts/release_gate.py) · [Self-test](#-self-test-eval-harness) |
...
| **Self-tested** | The team's own quality is regression-tested like code. | 1,900+ unit tests in CI (incl. the guards driven via their real protocol) + the eval harness: 9 rubrics, 51 golden cases, contract-checked in CI, live-scored by `/run-evals`. |
...
<details>
<summary>🧪 <b>What's in the harness</b>: 9 rubrics · 51 golden cases · deterministic scorer</summary>

- **9 rubrics** (code-review · coverage · spec/traceability · tuning · data-safety · process-discipline ·
  process-discipline-light · prompt-injection · regulatory-citation) + **51 golden cases** with deliberately seeded issues
...
evals/                          # team-quality eval harness: 9 rubrics + 51 golden cases
...
**Quality & evaluation**
- ✅ **Team-quality eval harness: SHIPPED (0.5.0)**. `evals/` has 9 rubrics + 51 golden cases
