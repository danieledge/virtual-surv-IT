...
def _mk_project(tmp_path, name="proj"):
    """A minimal CLOSED engagement pack that satisfies the current mechanical gate (the
    original fixture predated MISSING-INDEX/START-HERE checks and now trips them)."""
    p = tmp_path / name
    ...
    (p / "artifacts" / "START-HERE.html").write_text("x", encoding="utf-8")
    return p
...
def test_project_summary_reads_disk_state(tmp_path):
    ...
    s = project_summary(p)
    assert s["artifact_count"] == 2  # report.md + the START-HERE index the gate requires
