...
# --- the team's format (2026-09-14 user report) ------------------------------------------


def test_pack_uses_the_team_document_template(tmp_path):
    """The room used to carry a dark dashboard theme of its own, so the one artifact an
    auditor opens looked nothing like the documents beside it. It now goes through
    render_html.render_document: the same letterhead, house CSS and footer as every other
    HTML artifact, plus only the few classes its cards and chains need."""
    ...
    page = _rendered(ws)
    assert (
        '<header class="letterhead"><span class="brand">Compliance Surveillance Engineering' in page
    )
    assert '<div class="footer">' in page
    assert (
        "Generated by the compliance surveillance engineering team (AI agents, Virtual Surveillance IT)"
        in page
    )
    assert "<title>Evidence Room: Spoofing detection v2</title>" in page
    assert "<h1>Evidence Room: Spoofing detection v2</h1>" in page
    assert render_html._CSS.splitlines()[0] in page  # the house CSS, verbatim
    assert "@media (prefers-color-scheme: dark)" in page and "@media print" in page
    # the old theme is gone: its dark palette root, its own <footer>, its em dash
    assert "#0f1115" not in page and "<footer>" not in page
    assert "&mdash;" not in page and "\u2014" not in page and "\u2013" not in page
    # roster names read as AI agents, the team named in full
    assert "<b>\U0001f916 Amara</b>" in page and "<b>\U0001f916 Linh</b>" in page
    assert "all AI agents of Virtual Surveillance IT" in page
    # letterhead meta names the pack, not a date, so the render stays deterministic
    assert "Evidence Room · spoofing-v2 · status closed" in page
