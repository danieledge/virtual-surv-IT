...
- **`permissions.deny` has zero `Bash(...)` entries** (`.claude/settings.json:3-34`). Every
  Bash-based bypass below has no OS backstop - only the soft hook.

> **Live posture note (updated 0.3, re-worded 2026-07-30):** `.claude/.exec-consent` is
> git-ignored and absent from a fresh clone - the gate ships closed (a working machine may
> carry a local marker). Since 0.3 the model **cannot create it**: `guard-consent-writes.py`
