...
| The domain knowledge has to be typed in every session: typologies, MW79, SR 26-2, ATL/BTL method, EARS syntax… | Encoded once, cited to sources, and loaded only when relevant, with a register that grows instead of a prompt that gets retyped. |
