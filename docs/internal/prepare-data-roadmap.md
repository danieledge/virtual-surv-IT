...
| 2 | **NER redaction** (named-entity recognition - Presidio / spaCy) | Replace regex-only free-text masking with a PII model - catches **names, locations, orgs, obfuscated IDs**. Makes **comms/chat** data viable. | Med-High | Closes the #1 weakness (regex misses free-form PII). |
| 3 | **Format adapters** | Accept **CSV, Parquet, Excel, nested JSON** (auto-flatten) → normalise to the internal record shape. "Any structured file." | Med | Pure parsing; low risk. **Every adapter carries a built-in source-vs-output reconciliation** (counts + control total) - Excel extraction is the classic silent-truncation source (house rule in `docs/house-rules.md`; eval case `review-excel-truncation`). |
...
| 5 | **Validation hardening** | Run the NER scanner over the **output** to prove no PII leaked; add re-identification risk scoring - l-diversity and t-closeness (finer re-identification measures) layered on the existing k-anonymity (each record shares its identifying attributes with at least k-1 others). | Med | Makes "throw anything" *trustworthy*, not just possible. |
