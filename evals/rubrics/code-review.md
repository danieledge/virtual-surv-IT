...
|---|---|---|
| **Detection** | Every planted 🔴 Critical surfaced at the right severity; criticals not buried under nits. *(deterministic: recall + must-find)* | 0.30 |
| **No false alarms** | No false-positive trap flagged; correct, documented code not called a bug. *(deterministic: FP-traps)* | 0.15 |
| **Evidence basis** | Each finding tagged 📊 measured / 🧠 inferred; no inference presented as fact; file:line cited. | 0.125 |
| **Explanation & impact (5 C's)** | Each finding is written for a reader who was not in the session: plain-language Problem, a **Likely cause**, and an **Impact if unaddressed** in domain terms (detection gap / false negatives / alert volume / audit exposure). Naming the defect without cause + impact scores zero here. | 0.15 |
| **Actionability** | Each finding has a concrete, correct fix - not just "this is bad". | 0.125 |
...
| **Developer guidance** | The mandatory "improving future code" section is present and substantive. | 0.05 |
