...
`/remediate`), read `.claude/skills/<name>/SKILL.md` and follow it in this session, never the
...
6. **Present** in the shared `docs/review/output-format.md`: a clean traffic-light **scoreboard to
   ...
   flags it missing/empty as `FINDINGS-NO-DEV-GUIDANCE`, but verify it is in the
...
the audit above, the proportionate **Engagement Brief** (`.md` + `.html`, right-sized) written
