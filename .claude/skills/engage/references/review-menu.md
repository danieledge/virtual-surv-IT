...
`VSIT/shared/map.md` exists · `git ls-files | wc -l` in a repo (respects .gitignore) ·
last resort, a count-only `find`/`Get-ChildItem` excluding caches, `VSIT/engagements/` and
