...
6. **Enter the closing window**: `<python> -m scripts.engagement_state set-status closing
   ...
   `VSIT/engagements/<slug>/` and render to `.html` (`<python> -m scripts.render_html`). If the
...
7. **Engagement-summary email** (required closing artifact - Definition of Done): a short
   ...
   `VSIT/engagements/<slug>/`**, **signed off as Morgan** ("Hi," if you don't know the recipient's name). It's an
