...
Tool-by-tool caveats for the best-effort rows (TypeScript, Scala, Java, PowerShell) and the
record of what was removed and why (`semgrep`, `pip-audit`, the JVM build-driven analysers):
`docs/review/tooling.md`. Read it before driving any tool outside the eight supported ones;
the two rules that matter every time: never invoke `semgrep` or `pip-audit` even if present,
and never invoke `checkstyle`/`pmd` through `mvn`, `gradle` or `java -jar`.
