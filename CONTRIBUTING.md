...
1. On `dev`, run the **golden-slice `/run-evals`** (a representative ~10-15 of the 51 golden cases) in a
...
2. Record the result as **`evals/eval-baseline-<version>.md`** (date · cases run · pass/fail ·
   ...
   **The baseline MUST carry exactly one machine-readable ` ```eval-verdict ` block** (format
   ...
   Adjudicating a raw failure to a pass is still allowed, but it must be **counted** in the block
   (`cases_adjudicated_pass`) rather than absorbed into prose, and `verdict: pass` is reserved for
